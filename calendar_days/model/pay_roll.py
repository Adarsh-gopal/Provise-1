...
class HrPayInherit(models.Model):
    ...
    unpaid_id = fields.Float('Unpaid', strore=True, compute="_compute_week")
    # unpaid_id_1 = fields.Float(related = 'unpaid_id', string='Unpaid', store=True)
        ...
        for l in self:
            if l.date_from:
                if l.date_to:
                    ...
                    if holidays:
                        for hol in holidays:
                            unp = unp + hol.number_of_days_display
                        l.unpaid_id = unp
                    else:
                        l.unpaid_id =  unp
...
class unpaid_leaves(models.Model):
    ...
    unpaid_condition = fields.Boolean(index=True, store=True, default=True,related='holiday_status_id.unpaid')
