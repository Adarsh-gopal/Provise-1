...
class ReportAdgedPartner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        res = super(ReportAdgedPartner, self)._get_columns_name(options)
        column = {'name': _("Sales Person"), 'class': '', 'style': 'text-align:center; white-space:nowrap;'}
        res.insert(1, column)
        return res
    ...
    def _get_lines(self, options, line_id=None):
        ...
        context.update(user_ids=options.get('salespersion_ids', []))
            ...
            if 'partner_%s' % (values['partner_id'],) in options.get('unfolded_lines'):
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in [aml.move_id.invoice_user_id.name, format_date(self.env, aml.date_maturity or aml.date), aml.journal_id.code, aml.account_id.display_name, format_date(self.env, aml.expected_pay_date)]] +
    ...
    def _init_filter_partner(self, options, previous_options=None):
        ...
        options['salespersion_ids'] = previous_options and previous_options.get('salespersion_ids') or []
        selected_partner_ids = [int(partner) for partner in options['partner_ids']]
        selected_salesperson_ids = [int(user) for user in options['salespersion_ids']]
        selected_partners = selected_partner_ids and self.env['res.partner'].browse(selected_partner_ids) or self.env['res.partner']
        selected_users = selected_salesperson_ids and self.env['res.users'].browse(selected_salesperson_ids) or self.env['res.users']
        options['selected_partner_ids'] = selected_users.mapped('name')
        options['selected_salesperson_ids'] = selected_partners.mapped('name')
    ...
    def get_report_informations(self, options):
        if options and options.get('partner'):
            options['selected_salesperson_ids'] = options['salespersion_ids'] and [self.env['res.users'].browse(int(user)).name for user in options['salespersion_ids']] or []
        return super(ReportAdgedPartner, self._with_correct_filters1()).get_report_informations(options)
